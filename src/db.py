...
class DBAction: 
    ...
    def write(self, name, request:dict):
        try: 
            assert isinstance(request['conf'], list) # Check if request has confidence score
            ...
            self.cursor.execute(f'''INSERT INTO detectresult(name, class, conf) 
                                VALUES(?, ?, ?)''', (name, class_labels, conf_scores))
        except Exception as e: 
            raise e
